...
class BPlusTreeTest {
    ...
    @ParameterizedTest
    @CsvFileSource(resources = "/search-test-data.csv", numLinesToSkip = 1)
    @DisplayName("Тест поиска ключей")
    void testSearch(boolean shouldFind, int key, String keysToInsertStr) {
        // строку -> в массив int
        int[] keysToInsert = Arrays.stream(keysToInsertStr.replaceAll("[\\[\\]]", "")
                        .split(","))
                .map(String::trim)
...
//    }
    ...
    void testMultipleInserts() {
        ...
        for (int key = 1; key <= 10; key++) {
            assertTrue(tree.search(key), "Ключ " + key + " должен быть в дереве");
        }
